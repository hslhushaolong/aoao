...
class CoreSearch extends React.Component {
  constructor(props) {
    ...
    this.private = {
      isRenderMultiOperations: (dot.get(props, 'items.length') > 2),   // 判断搜索条件是否超过2个，超过则渲染多行的搜索选项
    };
  }
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    this.private.isRenderMultiOperations = (dot.get(nextProps, 'items.length') > 2);        // 判断搜索条件是否超过3个，超过则渲染多行的搜索选项
  }
  ...
  renderFormItems = () => {
    ...
    // 默认不展开的情况下，显示的项目不超过8条
    let formItems = _.cloneDeep(items);
    if (items.length > 8 && expand !== true) {
      formItems = formItems.slice(0, 8);
    }
