...
import { KnightSalaryType, Position, FillingType, DeductSubmitType, PersonalCutEventType, DutyState, renderReplaceAmount } from '../../../../../application/define';
...
class Create extends Component {
      ...
      columns: [{
        ...
        render: (text, record, tIndex) => {
          return (
            ...
            >
              {Object.keys(PersonalCutEventType).map((key, index) => {
                if (key === 'description') return false;
                return Object.hasOwnProperty.call(PersonalCutEventType, key) ? <Option key={index} value={`${PersonalCutEventType[key]}`}>{PersonalCutEventType.description(PersonalCutEventType[key])}</Option> : '';
              })}
            </Select>
          );
        },
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({
      ...
      detailErr: nextProps.salaryModel.detailErr, // 是否显示错误的modal
    });
    // // 页面结束的时候，注销刷新
  ...
  getTypeKey = (value) => {
    let keyName = '';
    Object.keys(PersonalCutEventType).map((item) => {
      if (PersonalCutEventType[item] === Number(value)) {
        keyName = item;
        return false;
      }
    });
  ...
  handleOk = () => {
    ...
    values.fund_id = KnightSalaryType.personnalDeduct;  // 扣款
  ...
  downloadExcel = () => {
    this.props.form.validateFields((err, values) => {
      if (err) {
        ...
        value.fund_id = KnightSalaryType.deduction; // 扣款
        this.props.dispatch({
  ...
  handleFileChange(ev) {
    ...
    reader.onload = (ev) => {
      ...
      const headStr = '城市,商圈名称,骑士姓名,手机号,补款项目,金额,原因,日期';
      ...
      this.setState({
        excelData,
        totalName,
        total: excelData[0].sheetContent.length,
      });
    };
  ...
  onFillingTypeChange = (value) => {
    this.setState({
      showExcelButton: value === `${FillingType.excelUpload}`, // 控制显示上传按钮
    });
  }
  ...
  onDownload = () => {
    ...
    dispatch({
      ...
      payload: {
        ...
        fund_item: PersonalCutEventType.description(Number(search.cutEvent)), // 款项类型中文字符串
  ...
  renderStatisticsPage = () => {
    return <Statistic data={this.state.manualDataSource} event={PersonalCutEventType} />;
  }
