    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              <a target="_blank" href={`/#/Salary/Manage/Knight/Supplement/Verify/Detail?detail_list=${record.detail_id_list}&month=${record.month}&submitState=${record.state}`} style={{ marginRight: '10px' }}>查看</a>
            </div>
          );
        },
      }];
