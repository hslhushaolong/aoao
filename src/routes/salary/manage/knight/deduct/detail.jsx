...
import { KnightSalaryApproveState, CutEventType, DeductSubmitType, KnightSalaryType, renderReplaceAmount } from '../../../../../application/define/index';
...
class Detail extends Component {
  ...
  renderSummary = () => {
    ...
    return (
      <div>
          ...
          <Row type="flex">
            ...
            }
            {
              Object.keys(obj).map((val, k) => {
                return <Col className="mgt16" key={k} style={style}>{CutEventType.description(CutEventType[val])}</Col>;
              })
            }
          </Row>
          <Row type="flex">
        ...
        {
          Number(this.props.location.query.state) !== DeductSubmitType.waitForSubmit ?
            <Button type="primary" size={'large'} style={{ width: '85px', marginLeft: '10px' }} onClick={this.onClickSelectedRows}>提交</Button> : ''
        }
  ...
  renderContent = () => {
    ...
    let columns = [{
      ...
      render: (text) => {
        return (
          <span>{CutEventType.description(CutEventType[text])}</span>
        );
      },
    }, {
      title: '扣款金额／元',
    ...
    if (Number(this.props.location.query.state) !== DeductSubmitType.waitForSubmit) {
          ...
          switch (record.state) {
            ...
            case KnightSalaryApproveState.reject: return (
              <div>
                ...
                <Popconfirm title="确定执行操作？" onConfirm={() => this.onClickSubmit(record._id)} okText="确定" cancelText="取消">
      ...
      return (
        <CoreContent style={{ marginTop: '16px' }}>
          <Table rowSelection={(Number(this.props.location.query.state) !== DeductSubmitType.waitForSubmit) && rowSelection} columns={columns} dataSource={this.state.dataSource} pagination={false} />
        </CoreContent>
      );
    }
    return (
