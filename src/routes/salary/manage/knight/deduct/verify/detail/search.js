...
import { KnightSalaryApproveState, KnightSalaryType, CutEventType } from '../../../../../../../application/define';
...
class Search extends Component {
  ...
  onReset = () => {
    ...
    const defaultParam = { fund_id: KnightSalaryType.deduction, sort: -1 };
    const params = {
  ...
  onSearch = (values) => {
    ...
    const { platform, city, district, supplier, complateState, submitDate, submitPeople, deductRange, knightName, handlType } = values;
    const params = {
      platform, // 平台
      city, // 城市
      district, // 商圈
      ...
      },
      {
        ...
        form: form => (form.getFieldDecorator('handlType')(
          <Select allowClear showSearch optionFilterProp="children" mode="multiple" placeholder="请选择扣款项目" >
            <Option value="cut_platform_offline">{CutEventType.description(CutEventType.cut_platform_offline)}</Option>
            <Option value="cut_accommodation">{CutEventType.description(CutEventType.cut_accommodation)}</Option>
            <Option value="cut_energy_charge">{CutEventType.description(CutEventType.cut_energy_charge)}</Option>
            <Option value="cut_stipulate">{CutEventType.description(CutEventType.cut_stipulate)}</Option>
            <Option value="cut_casualty">{CutEventType.description(CutEventType.cut_casualty)}</Option>
            <Option value="cut_social_security">{CutEventType.description(CutEventType.cut_social_security)}</Option>
            <Option value="cut_electrombile">{CutEventType.description(CutEventType.cut_electrombile)}</Option>
            <Option value="cut_equip">{CutEventType.description(CutEventType.cut_equip)}</Option>
            <Option value="cut_equip_cash_pledge">{CutEventType.description(CutEventType.cut_equip_cash_pledge)}</Option>
            <Option value="cut_other">{CutEventType.description(CutEventType.cut_other)}</Option>
          </Select>,
        )),
      },
      {
