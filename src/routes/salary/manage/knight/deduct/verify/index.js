...
class Index extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      knightChargeSource: props.knightChargeSource,   // 数据源 // TODO: 什么数据
    };
    this.private = {
  ...
  renderContent = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        title: '操作',
        dataIndex: 'detail_id_list',
        key: 'operation',
        render: (text, record) => {
          return (
            <div>
              <a target="_blank" href={`/#/Salary/Manage/Knight/Deduct/Verify/Detail?detail_list=${record.detail_id_list}&month=${record.month}&submitState=${record.state}`} style={{ marginRight: '10px' }}>查看</a>
            </div>
          );
        },
      }];
