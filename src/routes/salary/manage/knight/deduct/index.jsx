...
class IndexPage extends Component {
  ...
  onSearch = (params) => {
    ...
    const tempParams = {
      ...
      fund_id: KnightSalaryType.deduction, // 扣款
      page: params.page, // 页码
      limit: 30, // 每页条数
      sort: -1, // 排序
      platform_code: params.platform, // 平台code
      supplier_id: params.supplier, // 供应商id
      city_spelling_list: params.city, // 城市
      biz_district_list: params.district, // 商圈
      submit_state: Number(params.complateState), // 完成状态
      created_start_date: params.createdStartDate, // 创建开始日期
      created_end_date: params.createdEndDate, // 创建结束日期
    };
  ...
  onEdit = (index, id, list) => {
    ...
    const params = {
      fund_id: KnightSalaryType.deduction, // 骑士扣款类型
      page: 1, // 页码
      limit: 50, // 每页条数
      platform_code: this.private.searchParams.platform_code || platform, // 平台
      detail_id_list: list, // 详情ID列表
    };
    this.props.dispatch({
  ...
  onShowTaskCreateModal = () => {
    this.props.dispatch({
      ...
      payload: {
        hash: '/Salary/Manage/Knight/Deduct/Create', // 新建骑士扣款路由
      },
    });
  }
  ...
  renderContent = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <div>
              <Link target="_blank" to={`Salary/Manage/Knight/Deduct/Detail?id=${record._id}&detail_list=${record.detail_list}&state=${record.state}`}>查看</Link>
    ...
    return (
      <CoreContent>
        ...
        {/* 是否有新建骑士扣款按钮权限 */}
        { Operate.canOperateSalaryDeductionsCreateButton() ?
          <div style={{ marginBottom: '10px' }} >
            <Button type="primary" style={{ marginRight: '10px' }} onClick={this.onShowTaskCreateModal}>新建骑士扣款</Button>
          </div> : ''
        }
