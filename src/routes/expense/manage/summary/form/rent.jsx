...
class SummaryFormRent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      summaryId: props.summaryId, // 汇总记录id
  ...
  onClickCreate = () => {
    ...
    // 跳转到创建页面
    window.location.href = (`/#/Expense/Manage/Template/Create?type=${ExpenseType.rent}&summaryId=${summaryId}&name=${detail.costclass_name}&id=${detail.costclass_id}`);
  }
    ...
    const formItems = [
      {
        ...
        label: '费用类型',
        form: dot.get(detail, 'costclass_name', ExpenseType.description(ExpenseType.rent)),
      },
      {
        label: '审批流程',
        form: (
  ...
  judgeRoute = (state, recordId) => {
    if (state === ExpenseHouseState.new) {
      return (<a href={`/#/Expense/Manage/Template/Update?recordId=${recordId}&type=${ExpenseType.rent}`} style={{ marginRight: '10px' }}>编辑</a>);
    } else {
      return (<a href={`/#/Expense/Manage/Records/Form?recordId=${recordId}&houseState=${state}`} style={{ marginRight: '10px' }}>编辑</a>);
    }
  }

  // 是否隐藏新建按钮，断租／续租／续签／退租
  hideCreateButton = () => {
    const List = dot.get(this.props, 'approval.typeApplyList.data', []);
    if (List.length >= 1) {
      // 判断如果房租状态是，断租／续租／续签／退租
      if (List[0].thing_state == ExpenseHouseState.cancel || List[0].thing_state == ExpenseHouseState.break || List[0].thing_state == ExpenseHouseState.continue || List[0].thing_state == ExpenseHouseState.sign) {
        return false;
      }
    }
    return true;
  ...
  renderListInfo = () => {
    ...
    },
    {
      ...
      render: (text) => {
        return (
          <div>{ExpenseHouseState.description(text)}</div>
        );
      },
    }, {
      title: '金额（元）',
    ...
    // 新建按钮
    const titleExt = (
      this.hideCreateButton() ? <Button type="primary" onClick={this.onClickCreate}>新建</Button> : ''
    );
